 ...
 *
 * // Sysadmin con filtro estático (sin depender del user)
 * allowIf(isSysadmin, { status: { equals: 'published' } })
 *
 * // OrgRole con filtro dinámico (usa el orgId del user)
 ...
 * read: allow(
 *   allowIf(isSysadmin),
 *   allowIf(hasOrgRole(['admin']), (args) => ({ organization: { equals: getOrgId(args.req.user) } })),
 * )
 */
export const allow =
  (...rules: Access[]): Access =>
